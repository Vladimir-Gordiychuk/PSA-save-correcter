...
Утилита версии 1.0 модифицирует только распакованные файлы сохранений (т.е. перед её использованием необходимо 
распаковать файл с помощью другой утилиты AttrConv.exe, которую можно найти в инструментарии для модинга [2, 3].

В утилите версии 1.1 добавлена фича автоматичесого вызвова распаковщика AttrConv.
Данная фича будет работать если выполнены следующие условия:
1. указан путь к исполняемому файлу AttrConv.exe указан в PSA.Saver.exe.config файле;
2. в свойствах исполняемого файла AttrConv.exe включен режим "Совместимость с Windows 95" (тестировал на Windows 10);
3. запускать утилиту с правами Администратора.
Если в данной фиче нет необходимости, то можно оставить путь к AttrConv в PSA.Saver.exe.config файле пустым (и права Администратора не потребуются).

По-умолчанию, все должно быть настроено корректно, но мои косяки не исключены, да и пользователь может модифицировать config на свой вкус.
